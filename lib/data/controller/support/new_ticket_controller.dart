  ...
  void submit() async {
    ...
    if (message.isEmpty) {
      CustomSnackBar.error(
        errorList: [MyStrings.messageRequired.tr],
      );
      return;
    }

    if (message.isEmpty) {
      CustomSnackBar.error(errorList: [MyStrings.subjectRequired.tr]);
      return;
    }
    ...
    if(responseModel.statusCode == 200){
      ...
      if(model.status?.toLowerCase() == MyStrings.success.toLowerCase()){
        ...
        CustomSnackBar.success(successList: [MyStrings.ticketCreateSuccessfully.tr]);
        clearSelectedData();
      } else{
        Get.back(result: "updated");
